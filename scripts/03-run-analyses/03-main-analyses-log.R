# Notes ----------------------------------------------------------------------------------
#   Goal:   Reproduce main analyses (synth DID, SCM, and DID) with logged outcomes
#   Time:   ~ ??? minutes with 10 cores (and 500 placebo replications)
...
#   - FBI standard sales (HG + LG + other + multiple)
  ...
  # Log the rate (already per 100k population)
  bgc_data[, rate := log(rate)]
  # Drop population
  bgc_data[, pop := NULL]
...
# Save estimate objects ------------------------------------------------------------------
  ...
  qsave(
    x = results_all,
    file = here('data', 'clean', 'analyses', 'results-log-all.qs'),
    preset = 'high'
  )
  qsave(
    x = results_nonpoc,
    file = here('data', 'clean', 'analyses', 'results-log-nonpoc.qs'),
    preset = 'high'
  )
  qsave(
    x = results_nonp2p,
    file = here('data', 'clean', 'analyses', 'results-log-nonp2p.qs'),
    preset = 'high'
  )
  qsave(
    x = results_nonborder,
    file = here('data', 'clean', 'analyses', 'results-log-nonborder.qs'),
    preset = 'high'
  )
  qsave(
    x = results_brady,
    file = here('data', 'clean', 'analyses', 'results-log-brady.qs'),
    preset = 'high'
  )
