...
# Output ---------------------------------------------------------------------------------
#   exhibits/tables/standardized/tab-00-sdid-main.tex
#   exhibits/tables/standardized/tab-02-methods.tex
#     exhibits/tables/standardized/tab-02a-methods.tex
#     exhibits/tables/standardized/tab-02b-methods.tex
...
# Load results ---------------------------------------------------------------------------
  # Load results
  results_all =
    here('data', 'clean', 'analyses', 'standardized', 'results-all.qs') |>
    qread()
  results_nonpoc =
    here('data', 'clean', 'analyses', 'standardized', 'results-nonpoc.qs') |>
    qread()
  results_nonp2p =
    here('data', 'clean', 'analyses', 'standardized', 'results-nonp2p.qs') |>
    qread()
  results_nonborder =
    here('data', 'clean', 'analyses', 'standardized', 'results-nonborder.qs') |>
    qread()
  results_brady =
    here('data', 'clean', 'analyses', 'standardized', 'results-brady.qs') |>
    qread()
...
# Table 0z: Synth DID results from one sample --------------------------------------------
  # Define desired spatial sample
  res = copy(results_nonborder)
  # Format estimate and standard error
  tab =
    rbind(
      res$est_sdid |> comma(.01),
      res$se_sdid |> comma(.01) |> wrap()
    )
  # Add cumulative effect
  tab = rbind(tab, res$accum_sdid |> comma(.01))
  ...
  tab =
    ...
    save_kable(here('exhibits', 'tables', 'standardized', 'tab-00-sdid-main.tex'))
  tab =
    here('exhibits', 'tables', 'standardized', 'tab-00-sdid-main.tex') |>
    readLines()
  ...
  writeLines(
    text = tab,
    con = here('exhibits', 'tables', 'standardized', 'tab-00-sdid-main.tex')
  )

# Table 2z: Comparing estimators ---------------------------------------------------------
  ...
  main_table =
    map_dfr(
      ...
      .f = function(i) {
        rbind(
          res[i, .(est_sdid, est_scm, est_did)] |> unlist() |> comma(.01),
          res[i, .(se_sdid, se_scm, se_did)] |> unlist() |> comma(.01) |> wrap()
        ) |>
        as.data.table()
      }
    )
  ...
  acc_table %<>% .[, lapply(.SD, comma, accuracy = .01)]
  setDT(acc_table)
...
# NOTE Some formatting done by hand on Overleaf
  main_table |>
    ...
    column_spec(1, italic = TRUE) |>
    save_kable(here('exhibits', 'tables', 'standardized', 'tab-02a-methods.tex'))
  acc_table |>
    kbl(format = 'latex', align = 'lcccc', col.names = c_names, booktabs = TRUE) |>
    column_spec(1, italic = TRUE) |>
    save_kable(here('exhibits', 'tables', 'standardized', 'tab-02b-methods.tex'))
  # Load saved tables
  t1 =
    here('exhibits', 'tables', 'standardized', 'tab-02a-methods.tex') |>
    readLines()
  t2 =
    here('exhibits', 'tables', 'standardized', 'tab-02b-methods.tex') |>
    readLines()
  # Add indentation
  ...
  writeLines(
    text = final_table,
    con = here('exhibits', 'tables', 'standardized', 'tab-02-methods.tex')
  )
